...
#[derive(Clone)]
pub struct OtlpTraceSender {
    target: OtlpTarget,
    endpoint: Endpoint,
}

impl OtlpTraceSender {
    pub fn new(params: &OtlpSenderParams, target: OtlpTarget) -> anyhow::Result<Self> {
        let endpoint = make_endpoint(params, &target)?;
        Ok(OtlpTraceSender { target, endpoint })
    }
}

#[async_trait]
impl OtlpSender<Vec<ResourceSpans>> for OtlpTraceSender {
    async fn send(&self, resource_spans: Vec<ResourceSpans>) -> Result<OtlpExportResponse, Error> {
        let mut tonic_request = Request::new(ExportTraceServiceRequest { resource_spans });
        ...
        match response.into_inner().partial_success {
            ...
            Some(partial_success) if partial_success.rejected_spans == 0 => {
            ...
            Some(partial_success) => Ok(OtlpExportResponse::PartialSuccess {
                message: partial_success.error_message,
                dropped_items: partial_success.rejected_spans,
            }),
        }
    }
}
